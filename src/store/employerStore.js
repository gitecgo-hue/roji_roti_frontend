...
const useEmployerStore = create((set, get) => ({
  ...
  updateEmployerProfile: async (profileData) => {
    ...
    const response = await axios.patch(`${api}/employers/profile_update`, payload, headers);
  ...
  uploadProfilePhoto: async (photoFile) => {
    ...
    const response = await axios.post(`${api}/employees/profile_photo_upload`, formData, headers);
  ...
  getDashboard: async () => {
    ...
    try {
      ...
      const headers = { headers: { Authorization: `Bearer ${token}` } };
      const response = await axios.get(`${api}/employers/dashboard`, headers);
      const payload = unwrapPayload(response.data);
  ...
  getMyJobs: async () => {
    ...
    try {
      ...
      const headers = { headers: { Authorization: `Bearer ${token}` } };
      const response = await axios.get(`${api}/employers/my_jobs`, headers);
  ...
  getJobApplicants: async (jobId) => {
    ...
    try {
      ...
      const headers = { headers: { Authorization: `Bearer ${token}` } };
      const response = await axios.get(`${api}/employers/jobs/${jobId}/applicants`, headers);
      const payload = unwrapPayload(response.data);
      set({ isLoading: false });
  ...
  getCandidateDetail: async (candidateId) => {
    ...
    try {
      const token = typeof window !== 'undefined' ? localStorage.getItem('token') : null;
      if (!token) { set({ isLoading: false }); return null; }
      const headers = { headers: { Authorization: `Bearer ${token}` } };
      const response = await axios.get(`${api}/candidate/${candidateId}`, headers);
  ...
  // Send OTP to update phone number (new phone)
  sendPhoneUpdateOtp: async (newPhone) => {
    set({ error: null, isLoading: true });
    try {
      const response = await axios.post(`${api}/employers/profile/send_phone_no_update_otp`, { new_phone: newPhone }, getAuthHeaders());
      set({ isLoading: false });
      return response;
    } catch (error) {
      const errorMsg = extractErrorMessage(error);
  ...
  updatePhoneNo: async (newPhone, otpCode) => {
    set({ error: null, isLoading: true });
    try {
      const response = await axios.patch(`${api}/employers/profile/phone_no_update`, { new_phone: newPhone, otp_code: otpCode }, getAuthHeaders());
      // refresh employer data
      await get().getEmployer();
      set({ isLoading: false });
      return response;
    } catch (error) {
      const errorMsg = extractErrorMessage(error);
  ...
  // Send OTP for email verification/update
  sendEmailOtp: async (email) => {
    set({ error: null, isLoading: true });
    try {
      const response = await axios.post(`${api}/employers/profile/email/send_otp`, { email }, getAuthHeaders());
      set({ isLoading: false });
      return response;
    } catch (error) {
      const errorMsg = extractErrorMessage(error);
  ...
  verifyAndUpdateEmail: async (email, otpCode) => {
    set({ error: null, isLoading: true });
    try {
      const response = await axios.patch(`${api}/employers/profile/email/verify_and_update`, { email, otp_code: otpCode }, getAuthHeaders());
      await get().getEmployer();
      set({ isLoading: false });
      return response;
    } catch (error) {
