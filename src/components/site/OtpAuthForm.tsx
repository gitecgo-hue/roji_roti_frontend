...
export function OtpAuthForm({ title, subtitle, ctaLabel, accent = "primary", onSuccess, showNameField = true, isSignup = false, appRole = "employee" }: OtpAuthFormProps) {
  const { reqOtp, verifyOtp, reqSignupOtp, verifySignupOtp, error, isLoading, clearError } = useAuthStore();
  ...
  useEffect(() => {
    // Clear any lingering global auth errors when this form mounts
    clearError?.();
    return () => clearError?.();
  }, []);
  
  // Sync and animate error appearance/disappearance
  useEffect(() => {
    const combined = localError || error;
    if (combined) {
      setErrorText(combined);
      // Ensure class change happens on next frame for CSS transition
      requestAnimationFrame(() => setShowError(true));
    } else {
      // hide with animation
      setShowError(false);
      const t = setTimeout(() => setErrorText(""), 300); // duration must match CSS transition
  ...
  // Handle Backspace to move focus to previous input when current empty
  const handleOtpKeyDown = (i: number, e: any) => {
    ...
    if (otp[i] === '') {
      if (i > 0) {
        const prevIdx = i - 1;
        const prevEl = document.getElementById(`otp-${prevIdx}`) as HTMLInputElement | null;
        if (prevEl) {
          const next = [...otp];
          next[prevIdx] = '';
          setOtp(next);
      ...
      // If current has a value, clear it (standard backspace)
      const next = [...otp];
      next[i] = '';
      setOtp(next);
      e.preventDefault?.();
  ...
  // Handle Enter key to submit actions: Send OTP (mobile step) or Verify (otp step)
  const handleEnterKeyDown = (e: any) => {
    if (e.key !== 'Enter') return;
    // Prevent default form submission or other handlers
    e.preventDefault?.();
    if (isLoading) return;
    ...
    <div onKeyDown={handleEnterKeyDown}>
      ...
      {step === "mobile" ? (
        <div className="mt-6 space-y-4">
            ...
            <div className="mt-1.5 flex items-center gap-2 rounded-2xl bg-slate-100 px-4 py-3 shadow-card border border-slate-200">
              ...
              <input
                ...
                onChange={(e) => {
                  ...
                  setMobile(v);
                  // clear any global or local errors as user edits the number
                  clearError?.();
                  setLocalError("");
                }}
              />
          ...
          {errorText && (
            <div className={`rounded-2xl bg-red-50 p-3 flex gap-2 items-start transform transition-all duration-300 ${showError ? 'opacity-100 translate-y-0' : 'opacity-0 -translate-y-2 pointer-events-none'}`}>
        ...
        <div className="mt-6 space-y-4">
          <button
            onClick={() => {
              setStep("mobile");
              // clear errors when switching back to mobile entry
              clearError?.();
              setLocalError("");
            }}
