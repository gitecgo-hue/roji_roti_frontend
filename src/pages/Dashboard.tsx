...
import { Clock, MapPin, CalendarDays, FileText, Briefcase, User, Bookmark, CheckCircle2, Upload, ArrowRight, Edit3, CircleX, BookOpen, Settings2, Trash2,UploadCloud,FileUp } from "lucide-react";
...
export default function Dashboard() {
          ...
          <TabsContent value="resume" className="rounded-[2rem] bg-white p-0 shadow-sm mt-25 sm:mt-6">
            <div className="grid gap-6 lg:grid-cols-[320px_1fr]">
              ...
              <div className="space-y-6 p-6">
               ...
               <div className="rounded-[2rem] border border-slate-200 bg-white p-5 shadow-[0_20px_50px_-30px_rgba(15,23,42,0.3)] sm:p-6">
  {/* Upload Area */}
  <div className="rounded-[1.5rem] border border-dashed border-slate-300 bg-slate-50/70 px-5 py-8 text-center transition-all duration-300 hover:border-slate-400 hover:bg-slate-50">
    
    {/* Icon */}
    <div className="mx-auto mb-4 flex h-16 w-16 items-center justify-center rounded-2xl bg-white shadow-[0_10px_30px_-15px_rgba(15,23,42,0.35)]">
    ...
    <p className="mx-auto mt-1 max-w-sm text-sm leading-5 text-slate-500">
      Upload your latest resume in PDF format to keep your profile updated.
    </p>
    ...
    <div className="mt-5">
      <label
        ...
        className="inline-flex cursor-pointer items-center gap-2 rounded-xl bg-slate-900 px-5 py-2.5 text-sm font-semibold text-white shadow-sm transition-all duration-200 hover:bg-slate-800 hover:shadow-md active:scale-[0.98]"
      >
    ...
    <div className="mt-4 min-h-[24px]">
      {selectedFileName ? (
        <div className="mx-auto inline-flex max-w-full items-center gap-2 rounded-lg bg-white px-3 py-2 shadow-sm">
          <FileText className="h-4 w-4 shrink-0 text-slate-600" />

          <span
            className="max-w-[240px] truncate text-xs font-medium text-slate-700"
        ...
        <p className="text-xs text-slate-400">
          PDF only • Maximum file size supported
        </p>
      )}
    </div>
  </div>
  ...
  <button
    ...
    className={`mt-5 flex h-12 w-full items-center justify-center gap-2 rounded-xl bg-gradient-primary text-sm font-semibold text-white shadow-soft transition-all duration-200 hover:opacity-95 hover:shadow-lg active:scale-[0.99] disabled:cursor-not-allowed disabled:opacity-60 ${
      uploadingResume ? "animate-pulse" : ""
    }`}
  >
    <Upload
      className={`h-4 w-4 ${
        uploadingResume ? "animate-bounce" : ""
      }`}
    />
